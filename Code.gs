...
function getSheet() {
  ...
  if (!sheet) throw new Error(`Sheet "${CONFIG.SHEET_NAME}" not found. Please create it.`);
  return sheet;
}
...
function getVideos() {
  ...
  data.forEach((row, index) => {
      const id = row[0];
      const date = row[17];
      const hasContent = row.slice(1).some(cell => cell.toString().trim() !== ''); // Check Col B onwards
...
function saveVideo(data) {
  ...
  if (rowIndex === -1) {
    ...
    sheet.appendRow([
        data.id, data.title_de, data.text_de, data.title_en, data.text_en,
        data.ideas, data.tags, data.ref_image, data.source_link,
        data.director_notes, data.editor_instructions, data.raw_footage_url,
        data.final_video_url, data.link_youtube, data.link_instagram,
        data.link_tiktok, data.status, data.date
    ]);
  } else {
    ...
    const rowArray = [
        ...
        data.director_notes, data.editor_instructions, data.raw_footage_url,
        data.final_video_url, data.link_youtube, data.link_instagram,
        data.link_tiktok, data.status, data.date || new Date()
    ];
